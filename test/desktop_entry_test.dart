...
final manualDefinitionExample = DesktopFileContents(
    entry: DesktopEntry(
        ...
        unrecognisedEntries: <UnrecognisedEntry>[
          UnrecognisedEntry(
              ...
              comments: <String>[]),
          UnrecognisedEntry(
              key: 'X-Desktop-File-Install-Version',
              values: ['0.26'],
              comments: [''])
        ],
    ...
    unrecognisedGroups: <UnrecognisedGroup>[
      UnrecognisedGroup(
          group: DesktopGroup('X-UnknownGroup', comments: <String>['']),
      ...
      UnrecognisedGroup(
        group: DesktopGroup('X-EmptyGroup', comments: <String>['']),
        entries: <UnrecognisedEntry>[],
      ),
    ],
    actions: <DesktopAction>[
      DesktopAction(
        group: DesktopGroup('Desktop Action Gallery',
      ...
      DesktopAction(
          group: DesktopGroup('Desktop Action Create', comments: ['']),
...
final firefoxDefinition = DesktopFileContents(
    entry: DesktopEntry(
        version: SpecificationString('1.0'),
        name: SpecificationLocaleString('Firefox Web Browser',
        ...
        comment: SpecificationLocaleString('Browse the World Wide Web',
            localisedValues: <String, SpecificationLocaleString>{
              'ar':
                  SpecificationLocaleString('تصفح الشبكة العنكبوتية العالمية'),
              'ca': SpecificationLocaleString('Navegueu per la web'),
              'zh_TW': SpecificationLocaleString('瀏覽網際網路')
            }),
        genericName: SpecificationLocaleString('Web Browser',
            localisedValues: <String, SpecificationLocaleString>{
        ...
        mimeType: SpecificationTypeList(
            'text/html;text/xml;application/xhtml+xml;application/xml;application/rss+xml;application/rdf+xml;image/gif;image/jpeg;image/png;x-scheme-handler/http;x-scheme-handler/https;x-scheme-handler/ftp;x-scheme-handler/chrome;video/webm;application/x-xpinstall'.split(';').map((e) => SpecificationString(e)).toList(growable: false)),
        startupNotify: SpecificationBoolean(true),
        actions: SpecificationTypeList<SpecificationString>('NewWindow;NewPrivateWindow'.split(';').map((e) => SpecificationString(e)).toList(growable: false))),
    actions: <DesktopAction>[
      DesktopAction(
          group: DesktopGroup('Desktop Action NewWindow', comments: ['']),
          name:
              SpecificationLocaleString('Open a New Window', localisedValues: {
      ...
      DesktopAction(
          group:
              DesktopGroup('Desktop Action NewPrivateWindow', comments: ['']),
          name: SpecificationLocaleString('Open a New Private Window',
              localisedValues: {
                'ar':
                    SpecificationLocaleString('افتح نافذة جديدة للتصفح الخاص'),
                'ca': SpecificationLocaleString(
...
void main() async {
  test('Check XDG Directory Exists', () async {
    ...
    for (var element in dataDirs) {
      final pathToApplicationSubfolder =
          pathContext.join(element.path, 'applications');
      log(pathToApplicationSubfolder);
      existsLookup[pathToApplicationSubfolder] =
          Directory(pathToApplicationSubfolder).existsSync();
  ...
  test('Install Desktop Entry - Existing', () async {
    ...
    await installDesktopFileFromFile(existingFile,
        installationDirectoryPath: destinationFolderPath);

    final destinationFilePath =
        pathContext.join(destinationFolderPath, filename);
  ...
  test('Uninstall Desktop Entry - Existing', () async {
    ...
    final pathContext = Context(style: Style.posix);
    final sourceFilePath = pathContext.join(Platform.environment['HOME']!,
        localUserDesktopEntryInstallationDirectoryPath, filename);
  ...
  test('Parse Simple Desktop File Correctly', () async {
    ...
    const existingPath = 'test/$filename';
    final existingFile = File(existingPath);
    final DesktopFileContents contents =
        DesktopFileContents.fromFile(existingFile);
  ...
  test('Write To File Correctly', () async {
    ...
    final file = await DesktopFileContents.toFile(
        dir, filename, manualDefinitionExample);
  ...
  test('Write `example-firefox.desktop` to File Correctly', () async {
    ...
    final dir = await getTemporaryDirectory();
    final file =
        await DesktopFileContents.toFile(dir, filename, firefoxDefinition);
    final contentsFromFile = DesktopFileContents.fromFile(file);
